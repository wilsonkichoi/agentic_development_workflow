# Phase 2: Specification & Architecture — Prompt Template

## When to Use

After Phase 1 research is reviewed and approved. Before any code is written.

## Recommended Model

Highest reasoning available. Architecture decisions have the most downstream leverage.
- Claude Code: `claude --permission-mode plan --model opus`
- Copilot CLI: `/model claude-opus-4.6` then `/plan`

## Important

Use plan/read-only mode. Do NOT let the model generate implementation code.
...
{{OPTIONAL — For complex projects, additional roles review the spec in separate sessions after initial generation. See templates/roles/README.md for triggers:
- Compliance/auth/PII → Security Reviewer (templates/roles/security-reviewer.md)
- Complex business rules → Domain Specialist (templates/roles/domain-specialist.md)
- User-facing features → Product Reviewer (templates/roles/product-reviewer.md)}}
